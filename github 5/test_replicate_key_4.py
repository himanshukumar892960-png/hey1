...
APP_TOKEN = os.getenv("REPLICATE_API_TOKEN_4")

if not APP_TOKEN:
    print("Error: REPLICATE_API_TOKEN_4 not found in .env")
    exit(1)

print(f"Testing Quaternary Replicate API Token: {APP_TOKEN[:5]}...{APP_TOKEN[-5:]}")
...
try:
    ...
    if response.status_code == 200:
        data = response.json()
        print("✅ SUCCESS: Quaternary API Key is valid!")
